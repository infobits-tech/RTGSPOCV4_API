...
{
    [RoutePrefix("api/Approval/PoolManagement")]
    public class PoolManagementController : ApiController
    {
        ...
        [HttpGet]
        [Route("GetPoolDocs/{userId}")]
        public IHttpActionResult GetPoolDocs(int userId)
        {
            try
            {
                using (PoolManagementServices obj = new PoolManagementServices())
                {
                    return Ok(obj.GetPoolDocs(userId));
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        [Route("UpdateStatusForDocument/{doc_typ_sk}/{doc_mst_sk}/{status_sk}/{user_sk}")]
        public IHttpActionResult UpdateStatusForDocument(int doc_typ_sk, int doc_mst_sk, int status_sk, int user_sk)
        {
            try
            {
                using (PoolManagementServices obj = new PoolManagementServices())
                {
                    return Ok(obj.UpdateStatusForDocument(doc_typ_sk, doc_mst_sk, status_sk, user_sk));
                }
            }
            catch (Exception ex)
            {
