...
{
    public class Inwardmt202Services : IDisposable
    {
        ...
        public List<VU_INBOUND_TXN_DTL> GetData(int user_sk)
        {
            try
            {
                using (Connection db = new Connection())
                {
                    ...
                    // Get user aprv dt
                    DataTable dt = db.getDataTable("select * from STP_ENG.Inbound_TXN_DTL_vu where doc_typ_sk=1003 order by TXN_DTE_TME desc", con);
